...
class FileComparison:
  ...
  def assertEqual(testcase, file1, file2, msg = ''):
    ...
    if f1istext != f2istext:
      raise Exception( 'Comparison is not possible because files %s and %s are not of the same type' % ( file1, file2 ) )
    elif f1istext :
...
class CommandsTestManager(unittest.TestCase):
  ...
  def create_ref_directory( self, testcommand ):
    test_ref_directory = os.path.join( self.ref_directory, testcommand.get_test_name() )
  ...
  def create_run_directory( self, testcommand ):
    test_directory = os.path.join( self.run_directory, testcommand.get_test_name() )
  ...
  def get_ref_files( self, testcommand ):
    return CommandsTestManager.get_command_files(testcommand, self.get_ref_directory(testcommand))
  ...
  def get_run_files( self, testcommand ):
    return CommandsTestManager.get_command_files(testcommand, self.get_run_directory(testcommand))
  ...
  def setUp(self):
    ...
    if hasattr(self, 'test_cases') :
      ...
      raise Exception( 'An attribute named test_cases that contains a list of CommandTest must be added.' )
