...
/*    h_64->calibration_units = h->calibration_units;*/
    ...
    if (h->patient_age) sprintf(h_64->patient_age,"%f",h->patient_age);
    if (h->patient_height) sprintf(h_64->patient_height,"%f",h->patient_height);
    if (h->patient_weight) sprintf(h_64->patient_weight,"%f",h->patient_weight);
