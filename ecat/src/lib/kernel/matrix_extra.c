 ...
 * 17-Apr-2000: comtat@ieee.org
 *      In the main header, the fields num_frames, num_gates, and bed_positions are
 *      set to the actual total number of frames, gates, and bed offset, even if the frame,
 *      gate, or bed offset numerotation is higher. For example, if only frame 7 is written,
 *      the the number of frame is set to one, and we can not use CAPP 7 software to
 *      access that frame. I changed mh_update so that the fields num_frames, num_gates, and
 *      bed_positions is set to the higher frame, gate, and bedd position number.
 *  19-sep-2002: 
 ...
 *  11-Apr-2000: comtat@ieee.org
 *    Modify mh_update such as to set num_frame to the highest frame number.
 *    Same for gate and bed offset
 * 19-May-2003: papadopo@shfj.cea.fr
 *      Compile with GCC 3.3: change <varargs.h> into <stdarg.h>.
 */
...
MatrixErrorCode matrix_errno = MAT_OK;
char		matrix_errtxt[132] = "\0";
	...
	MatrixFile     *file;
{
	...
	while (node != NULL) {
		...
		if (bsearch(&val.frame, frames, num_frames, sizeof(int), compare_int) ==
			...
			/* C. Comtat, 11.04.2006 */
			if (val.frame > frame_max) frame_max = val.frame;
		}
		...
		if (bsearch(&val.gate, gates, num_gates, sizeof(int), compare_int) ==
			...
			/* C. Comtat, 11.04.2006 */
			if (val.gate > gate_max) gate_max = val.gate;
		}
		if (bsearch(&val.bed, beds, num_beds, sizeof(int), compare_int) ==
			...
			/* C. Comtat, 11.04.2006 */
			if (val.bed > bed_max) bed_max = val.bed;
		}
		node = node->next;
	}

	/* C. Comtat, 17.04.2000 */
	if (mh->num_frames < frames[num_frames-1]) {
	  mh->num_frames = frames[num_frames-1];
	  mod++;
	}
	if (mh->num_gates < gates[num_gates-1]) {
	  mh->num_gates = gates[num_gates-1];
	  mod++;
	}
	if (mh->num_bed_pos < beds[num_beds-1]) {
	  mh->num_bed_pos = beds[num_beds-1];
	  mod++;
	}

	/* C. Comtat, 11.04.2006 */
	if (mh->num_frames > frame_max) {
	  mh->num_frames = frame_max;
	  mod++;
	}  
	if (mh->num_gates > gate_max) {
	  mh->num_gates = gate_max;
	  mod++;
	}  
	if (mh->num_bed_pos > bed_max) {
	  mh->num_bed_pos = bed_max;
	  mod++;
	}  
