...

{
  ...
  try {
    ...
    AimsApplication application(
      ...
      "above one, except for the -b, -g, -c and --old-mode options. So if you "
      "are not using these options, use AimsApplyTransform preferably, which "
      "is more powerful, and clearer to use.\n"
      "Typically to deform an image, the equivalent to:\n"
      "AimsRegisterFFDApply -i image.nii -o output.nii -d ffd.ima "
      "-m affine.trm\n"
      "will be:\n"
      "AimsApplyTransform -i image.nii -o output.nii -M inv:affine.trm "
      "-M ffd.ima --vi 3 -t 3\n"
      "\n\n"
