...
def _build_fitpoly_matrix(volume, order, mask, transformation=None):
...
def fit_poly_coefs(volume, mask, order):
...
def apply_poly(volume_like, mask, coefs, transformation=None):
    ...
    assert mask.shape == np.asarray(volume_like).shape[:3]
    ret = np.zeros_like(mask, dtype=float)
